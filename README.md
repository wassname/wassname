...
- **[Moral Maps](https://github.com/wassname/moral-maps): where do models sit among humans?**
  ...
  Where do models fall in terms of human culture, personality, and humour? I apply human surveys to LLMs and compare them with maps of human answers. On the World Values Survey I scored 17 frontier models by rated sampling, twelve ratings per item with the option order shuffled, and placed them among 90 human societies on the Inglehart-Welzel axes. They spread from 0.53 to 0.76 on the secular-rational axis, with gpt-5.5 furthest out at 0.76 ± 0.07, so the models really are somewhere west of Silicon Valley. Whether the newer ones keep voyaging is less clear: most families drift that way with each release, but the moves sit inside the 95% intervals I report for every model.
